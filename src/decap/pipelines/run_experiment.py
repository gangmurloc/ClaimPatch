...
from decap.baselines.descendant_all import descendant_all_labels, descendant_all_patch
from decap.baselines.attribute_no_graph import attribute_no_graph_labels, attribute_no_graph_patch
from decap.baselines.full_regeneration import full_regeneration_patch
from decap.baselines.independent_claim_revision import direct_only_labels, direct_only_patch
from decap.baselines.minimal_edit_prompt import unstructured_selective_edit_patch
...
from decap.data.validators import validate_instances
from decap.evaluation.bootstrap import paired_bootstrap_delta
from decap.evaluation.citation_metrics import citation_metrics
from decap.evaluation.cost_metrics import approximate_cost
from decap.evaluation.impact_metrics import impact_classification_metrics
from decap.evaluation.patch_metrics import patch_metrics
from decap.evaluation.preservation_metrics import preservation_metrics
...
from decap.pipelines.prompted_modules import PromptedGenerationError, PromptedPatchPipeline
...
def _shared_llm_client_for_run(systems: List[str], config: Dict[str, Any]) -> Any:
    """Reuse one local LLM for DECAP and graph-free LLM baselines.
...
def run_p0(config_path: Path, limit: int = None) -> Path:
    ...
    stage_title = str(config.get("stage_title", "DECAP P0 Rule-Based Synthetic Experiment"))
    interpretation = str(
        config.get(
            "interpretation",
    ...
    primary_system = str(config.get("primary_system", "decap_rule" if "decap_rule" in systems else systems[0]))
